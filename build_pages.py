...
<div class="strip" id="strip">
  <div class="cell glass cell-hero" id="needCell">
    <div class="k">Avg. need index <span class="live-tag"><span class="dot"></span>Live</span></div>
    <div class="v" id="avgNeed">—</div>
    <div class="gauge"><div class="gauge-track"><div class="gauge-marker" id="avgNeedMarker" style="left:0%%"></div></div>
    <div class="gauge-labels"><span>Watch</span><span>Elevated</span><span>High</span><span>Critical</span></div></div>
  </div>
  <div class="cell glass cell-accent-alert">
    <div class="k">Zones at Alert+ <span class="live-tag"><span class="dot"></span>Live</span></div>
    <div class="v" id="zonesAlert">—</div>
    <div class="zone-dots" id="zoneDots"></div>
    <div class="n">Of NDMA/IPC-monitored zones</div>
  </div>
  <div class="cell glass cell-accent-brand"><div class="k">PACIDA operational areas</div><div class="v">3<span style="font-size:14px;color:var(--muted)"> + Borena</span></div><div class="n">Marsabit, Samburu, Isiolo counties &amp; the Borena cross-border zone</div></div>
  <div class="cell glass cell-accent-static"><div class="k">Kenya households <span class="static-tag">2019 census</span></div><div class="v" id="totHH">—</div><div class="n">KNBS 2019 census, 3 counties</div></div>
  <div class="cell glass cell-accent-static"><div class="k">Kenya population <span class="static-tag">2019 census</span></div><div class="v" id="totPop">—</div><div class="n">KNBS 2019 census, 3 counties</div></div>
  <div class="cell glass cell-accent-brand"><div class="k">Last data refresh <span class="live-tag"><span class="dot"></span>Live</span></div><div class="v mono" id="lastRef" style="font-size:16px">—</div><div class="n">Auto-refreshes every 10 min</div></div>
</div>
...
<div class="entry-grid" style="padding-top:22px">
  ...
  <a class="entry-card glass" href="about.html">
...
<div class="wide">
  ...
  <div class="panel glass">
    ...
    <div class="resource-grid">
      ...
      <a class="resource-card" href="about.html#rating-framework"><b>Rating framework &amp; methodology</b><span>How the 0&ndash;100 Need Index is calculated and where the data comes from.</span></a>
...
async function refreshAll(){
  ...
  try{
    ...
    document.getElementById("zoneDots").innerHTML = asalZones.map(r=>{
      const band = r.live ? needBand(r.live.need) : "elevated";
      return `<span class="zd" style="background:${PHASE_COLORS[band]}" title="${r.name}: ${bandLabel(band)}"></span>`;
    }).join("");
    const avg = Math.round(ok.reduce((a,r)=>a+r.live.need,0)/ok.length);
    const avgBand = needBand(avg);
    document.getElementById("avgNeed").textContent = avg;
    document.getElementById("avgNeed").style.color = PHASE_COLORS[avgBand];
    document.getElementById("avgNeedMarker").style.left = avg + "%%";
